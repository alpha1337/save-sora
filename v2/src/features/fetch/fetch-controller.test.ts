...
describe("fetch-controller helpers", () => {
  ...
  it("stops a non-offset crawl when the cursor repeats and no new row keys appear", () => {
    expect(
      shouldStopForStalledCursor(
        "same-cursor",
        "same-cursor",
  ...
  it("does not stop a stalled-cursor page when new raw row keys are still appearing", () => {
    expect(
      shouldStopForStalledCursor(
        "same-cursor",
        "same-cursor",
